// NOTE: use uint32_t for all elements to always use 32 bits (modulo 2 ^ 32)
#include <iostream>
#include <string>
#include <vector>
#include <algorithm>
...
string SHA256::encrypt(string t) {
	...
	int original_length = msg.length() - 1;
	while (msg.length() % 512 != 448) msg += '0';

	msg += int2bin(original_length, 64);
	...
	for (auto Mi : M) {
		...
		for (int i = 0; i < 64; i++) {
			...
			// uncomment this to see each iteration
			/*cout << "t=" << i << " => ";
			cout << toHEX(a,8) << "  ";
			cout << toHEX(b,8) << "  ";
			cout << toHEX(c,8) << "  ";
	...
	// we put the hashes together to show the result
	string result = "";
	for (auto h : H) result += toHEX(h, 8);
	return result;
}
